...
> **版本**: 3.0  
> **更新日期**: 2026-04-27  
> **说明**: 评论相关接口已解耦到笔记评论管理模块，本模块不再包含评论相关接口。所有交互数据（浏览量、点赞数、收藏数）均通过 Redis 缓存服务获取。
...
> **注意**: 评论相关接口（获取评论列表、发表评论、评论点赞）已迁移至 `DocNoteCommentController.java`，详见 [笔记评论管理 API 接口文档]([document]笔记评论管理-api接口文档.md)。
...
- **功能**: 获取笔记详情，包含作者信息、统计数据。所有统计数据（浏览量、点赞数、收藏数）均从 Redis 缓存获取，不再依赖主表字段。
...
- **功能**: 独立查询笔记的交互统计数据，包括浏览量、点赞数、收藏数以及当前用户的交互状态。
