...
export default function CommentSection({ noteId }: CommentSectionProps) {
  ...
  useEffect(() => {
    ...
    const fetchComments = async () => {
      setLoading(true)
      try {
        const res = await getDocHomeNoteComments(noteId, {
          pageNum: 1,
          pageSize: 10,
          sort,
        })
  ...
  }, [data, loadingMore, pageNum, noteId, sort])
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      setInputText('')
      setReplyTo(null)
      const res = await getDocHomeNoteComments(noteId, { pageNum: 1, pageSize: 10, sort })
      setData(res)
      setPageNum(1)
    } catch {
      toast.error('评论发表失败')
  ...
  }, [inputText, isLoggedIn, noteId, replyTo, sort])
  ...
  const handleCommentLike = useCallback(
    async (commentId: string, isLiked: boolean) => {
      ...
      const updateItem = (list: DocHomeComment[]): DocHomeComment[] =>
        list.map((c) => {
          if (c.id === commentId) {
      ...
      try {
        await toggleDocHomeNoteCommentLike(commentId)
      } catch {
        setData((prev) => (prev ? { ...prev, list: updateItem(prev.list) } : prev))
        toast.error('操作失败')
      }
        ...
        <>
          ...
          <div className="mb-4">
            <AceternityTabs
              ...
              layoutIdNamespace="doc-comment-sort-pill"
            />
          </div>
