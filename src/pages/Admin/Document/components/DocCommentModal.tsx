 ...
 *
 * 功能：
 * - 根据文档ID查询该文档的所有评论（使用前台 /comments/{noteId} 接口）
...
import {
  getDocNoteComments,
  deleteDocNoteComment,
  updateDocNoteComment,
 ...
 * 获取评论状态对应的 Chip 颜色
 */
function getCommentStatusColor(status: DocCommentStatus): 'success' | 'warning' | 'danger' {
 ...
 * 获取评论状态的显示标签
 */
function getCommentStatusLabel(status: DocCommentStatus): string {
...
export interface DocCommentModalProps {
  isOpen: boolean
...
interface CommentItemProps {
  ...
  isReply?: boolean
  replyToName?: string
  onToggleStatus: (id: string, currentStatus: DocCommentStatus) => void
 ...
 * 文档评论管理弹窗组件
 */
export function DocCommentModal({ isOpen, onOpenChange, docId, docTitle }: DocCommentModalProps) {
  // 评论列表状态
  const [commentList, setCommentList] = useState<DocNoteComment[]>([])
  ...
  /**
   * 获取评论列表（使用前台 /comments/{noteId} 接口）
   */
  const fetchCommentList = useCallback(async () => {
    ...
    setIsLoading(true)
    try {
      const data: DocNoteCommentPageData = await getDocNoteComments(docId, page, pageSize, 'new')
  ...
  const handleToggleStatus = useCallback(
    async (id: string, currentStatus: DocCommentStatus) => {
      const newStatus: DocCommentStatus = currentStatus === 1 ? 2 : 1
      try {
        await updateDocNoteComment({
          id,
          status: newStatus,
        })
