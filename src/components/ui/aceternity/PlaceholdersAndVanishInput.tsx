    ...
    const animateFrame = (pos: number = 0) => {
      requestAnimationFrame(() => {
        ...
        for (let i = 0; i < newDataRef.current.length; i++) {
          const item = newDataRef.current[i];
          if (item.x < pos) {
            newArr.push(item);
          } else {
            if (item.r <= 0) continue;
            item.x += Math.random() > 0.5 ? 1 : -1;
            item.y += Math.random() > 0.5 ? 1 : -1;
            item.r -= 0.05 * Math.random();
            newArr.push(item);
          }
        }
  ...
  return (
    ...
    >
      ...
      <input
        ...
        type="text"
        className={cn(
          "w-full relative text-base sm:text-lg z-50 border-none dark:text-white bg-transparent text-black h-full rounded-full focus:outline-none focus:ring-0 pl-12 sm:pl-16 pr-20",
        ...
        <AnimatePresence mode="wait">
          {!value && (
            <motion.p
              ...
              className="dark:text-zinc-500 text-base sm:text-lg font-normal text-neutral-500 pl-12 sm:pl-16 text-left w-[calc(100%-2rem)] truncate"
            >
